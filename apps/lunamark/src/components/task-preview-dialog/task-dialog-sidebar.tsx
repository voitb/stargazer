...
import { Avatar, FormField, Input, Select } from "@ui/components";
	...
	return (
		<div className="w-72 flex-shrink-0 border-l border-[rgb(var(--color-neutral-stroke-1))/0.3] bg-[rgb(var(--color-neutral-background-2))/0.3] overflow-y-auto custom-scrollbar">
			<div className="p-5 space-y-5">
				...
				<FormField label="Status">
					{(inputProps) => (
						<Select
							...
							onValueChange={(value) => form.setStatusValue(value as TaskStatus)}
						>
				...
				<FormField label="Labels" description="Comma-separated values">
					{(inputProps) => (
						<Input
							{...inputProps}
				...
				<FormField label="Avatar URL" description="Auto-filled from GitHub username">
					{(inputProps) => (
						<Input
							{...inputProps}
							type="url"
