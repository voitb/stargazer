---
id: task-148
title: Create terminal size utilities
status: pending
priority: high
labels:
  - cli
...
Create terminal size utilities for non-hook usage.
Extracts terminal size detection for use outside React components.

## Reference

See: `packages/cli/CLI_DESIGN_SYSTEM.md` - Responsive Design section

## Acceptance Criteria

- [ ] Create `packages/cli/src/tui/design-system/utils/terminal-size.ts`
- [ ] Export getTerminalSize function
- [ ] Export breakpoint utilities
- [ ] Support stdout fallback dimensions
...
import {
  ...
  canShowFullLogo,
  truncateToFit,
} from '../design-system/utils/terminal-size.js';

// In startup code (non-React)
function printStartupBanner() {
  const { columns } = getTerminalSize();
  console.log(`Terminal: ${columns} columns`);

  if (canShowFullLogo()) {
    console.log(renderFullLogo());
  } else {
