---
id: task-149
title: Create tokens index and export all tokens
status: pending
priority: high
labels:
  - cli
  - design-system
  - tokens
created: '2025-01-09'
order: 149
assignee: glm
depends_on:
  - task-129
  - task-130
  - task-131
  - task-132
  - task-133
  - task-134
---

## Description

Create the tokens index file that exports all design tokens.
Central export point for the token system.

## Reference

See: `packages/cli/CLI_DESIGN_SYSTEM.md` - Design Tokens section

## Acceptance Criteria

- [ ] Create `packages/cli/src/tui/design-system/tokens/index.ts`
- [ ] Export all token modules
- [ ] Create combined tokens object
- [ ] Type exports for all tokens

## Implementation

**File**: `packages/cli/src/tui/design-system/tokens/index.ts`

```typescript
/**
 * Stargazer CLI Design System - Design Tokens
 *
 * Central export for all design tokens.
 *
 * @example
 * ```typescript
 * import { tokens, spacing, colors } from './tokens/index.js';
 *
 * // Use individual tokens
 * <Box padding={spacing.md} />
 *
 * // Or combined tokens object
 * <Box padding={tokens.spacing.md} />
 * ```
 */

// ═══════════════════════════════════════════════════════════════
// SPACING
// ═══════════════════════════════════════════════════════════════

export {
  spacing,
  spacingDefaults,
  getSpacing,
  type SpacingToken,
  type SpacingValue,
} from './spacing.js';

// ═══════════════════════════════════════════════════════════════
// TYPOGRAPHY
// ═══════════════════════════════════════════════════════════════

export {
  typography,
  getTypography,
  type TypographyStyle,
  type TypographyToken,
} from './typography.js';

// ═══════════════════════════════════════════════════════════════
// BORDERS
// ═══════════════════════════════════════════════════════════════

export {
  borderStyles,
  borderColors,
  borderPresets,
  dividerChars,
  createDivider,
  type BorderStyle,
  type BorderColor,
} from './borders.js';

// ═══════════════════════════════════════════════════════════════
// LAYERS
// ═══════════════════════════════════════════════════════════════

export {
  layers,
  getLayer,
  isAbove,
  sortByLayer,
  type LayerToken,
  type LayerValue,
} from './layers.js';

// ═══════════════════════════════════════════════════════════════
// MOTION
// ═══════════════════════════════════════════════════════════════

export {
  motion,
  duration,
  easing,
  spinnerConfig,
  typewriterConfig,
  pulseConfig,
  getSpinnerFrames,
  type DurationToken,
  type DurationValue,
  type EasingToken,
  type SpinnerType,
} from './motion.js';

// ═══════════════════════════════════════════════════════════════
// COLORS
// ═══════════════════════════════════════════════════════════════

export {
  colors,
  surfaceColors,
  textColors,
  borderColorsMap,
  statusColors,
  getThemeColors,
  type ThemeColors,
  type StatusType,
  // Re-exports from palettes
  PALETTES,
  THEMES,
  DEFAULT_PALETTE,
  STAR_ICONS,
  type Palette,
  type PaletteName,
  type ThemeName,
  type RGB,
} from './colors.js';

// ═══════════════════════════════════════════════════════════════
// COMBINED TOKENS
// ═══════════════════════════════════════════════════════════════

import { spacing } from './spacing.js';
import { typography } from './typography.js';
import { borderStyles, borderColors } from './borders.js';
import { layers } from './layers.js';
import { motion } from './motion.js';
import { colors, PALETTES, STAR_ICONS } from './colors.js';

/**
 * Combined tokens object for convenient access
 */
export const tokens = {
  spacing,
  typography,
  borders: {
    styles: borderStyles,
    colors: borderColors,
  },
  layers,
  motion,
  colors,
  palettes: PALETTES,
  icons: STAR_ICONS,
} as const;

export type Tokens = typeof tokens;
```

## Test

```bash
cd /Users/voitz/Projects/gemini-hackathon/packages/cli
pnpm exec tsc --noEmit
```

## Usage Example

```typescript
import { Box, Text } from 'ink';
import { tokens, spacing, typography, colors } from '../design-system/tokens/index.js';

function StyledCard({ children }) {
  return (
    <Box
      padding={tokens.spacing.md}
      borderStyle={tokens.borders.styles.round}
      borderColor={tokens.borders.colors.subtle}
    >
      <Text {...tokens.typography.title}>Title</Text>
      <Text {...tokens.typography.body}>{children}</Text>
    </Box>
  );
}

// Or with direct imports
function AltCard({ children }) {
  return (
    <Box padding={spacing.md}>
      <Text {...typography.title}>Title</Text>
      {children}
    </Box>
  );
}
```
