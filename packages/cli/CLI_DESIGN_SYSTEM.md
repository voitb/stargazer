...
├── CLI_DESIGN_SYSTEM.md     # This document
│
├── tokens/
...
│   └── theme-provider.tsx   # Theme context & detection
│
├── logo/
│   ├── index.ts
│   ├── ascii-art.ts         # Logo definitions (full/medium/compact/minimal)
│   ├── render-logo.tsx      # Logo component with gradient
│   ├── typewriter.tsx       # Animated intro effect
│   └── responsive.ts        # Terminal width detection
│
├── animations/
│   ├── index.ts
│   ├── star-spinner.tsx     # ✦→✧→☆→★ rotation
│   ├── pulse.tsx            # Gradient intensity oscillation
│   ├── typewriter.tsx       # Character-by-character reveal
│   └── transitions.tsx      # Screen transition effects
│
├── components/
│   ├── index.ts
...
| `layers` | Z-ordering | `layers.base`, `layers.overlay`, `layers.toast` |
...
```typescript
import { StarSpinner } from '../design-system/animations';
...
```typescript
import { Typewriter } from '../design-system/animations';
...
```typescript
import { PulseGlow } from '../design-system/animations';

<PulseGlow intensity={[0.5, 1.0]} duration={1000}>
  <Text>Pulsing text</Text>
</PulseGlow>
```
...
```typescript
import { useResponsiveLogo } from '../design-system/logo';
...
```typescript
import { AnimatedLogo } from '../design-system/logo';
...
```typescript
import { detectTheme, ThemeProvider } from '../design-system';
...
```typescript
import { useTerminalSize, useBreakpoint } from '../design-system/utils';
...
|------|------|-------------|
| task-129 | `tokens/spacing.ts` | Spacing scale (xs→xl) |
| task-130 | `tokens/typography.ts` | Typography tokens |
| task-131 | `tokens/borders.ts` | Border styles & patterns |
| task-132 | `tokens/layers.ts` | Z-index scale |
| task-133 | `tokens/motion.ts` | Animation timing tokens |

### Phase 2: Color Enhancement

| Task | File | Description |
|------|------|-------------|
| task-134 | `tokens/colors.ts` | Enhanced color system |
| task-135 | `primitives/theme-provider.tsx` | Theme context |

### Phase 3: Animations

| Task | File | Description |
|------|------|-------------|
| task-136 | `animations/star-spinner.tsx` | Star rotation spinner |
| task-137 | `animations/typewriter.tsx` | Text reveal effect |
| task-138 | `animations/pulse.tsx` | Pulse/glow effect |

### Phase 4: Logo System

| Task | File | Description |
|------|------|-------------|
| task-139 | `logo/responsive.ts` | Width detection |
| task-140 | `logo/render-logo.tsx` | Responsive logo component |
| task-141 | `logo/typewriter.tsx` | Animated logo intro |

### Phase 5: Components

| Task | File | Description |
|------|------|-------------|
| task-142 | `components/card.tsx` | Card component |
| task-143 | `components/divider.tsx` | Divider component |
| task-144 | `components/badge.tsx` | Badge component |
| task-145 | `components/key-hint.tsx` | KeyHint component |
| task-146 | `components/toast.tsx` | Toast component |
